...
# Utilisation d'un chemin absolu pour éviter les erreurs de création de fichier
basedir = os.path.abspath(os.path.dirname(__file__))
database_url = os.getenv('DATABASE_URL', 'postgresql://test:test@localhost:5432/meshora_db')
# Compat Dokku/Heroku: certaines URLs peuvent arriver en postgres://
...
class Intention(db.Model):
    ...
    id_transaction = db.Column(db.String(100), unique=False, nullable=True) # False pour permettre le groupage
...
@app.route('/')
def index():
    # L'index devient dynamique en affichant les paroisses disponibles
    paroisses = Paroisse.query.all()
...
def demande_messe(paroisse_id):
    ...
    special_disponible = False
    if paroisse.date_lite_special and maintenant <= paroisse.date_limite_special:
...
def traiter_demande_post():
    ...
    # On utilise la méthode par mots-clés, c'est la plus fiable
    if "Grâce" in nature_brute:
        ...
        n_id = "IS"
    # ATTENTION : Ne rien ajouter ici qui pourrait modifier n_id

    nom_donateur = request.form.get('nom')
    tel_donateur = request.form.get('telephone') # Récupération du numéro
        ...
        for i in range(1, nb_jours + 1):
            ...
            if d_brute and h_brute:
                ...
                nouvelle_intention = Intention(
                    ...
                    telephone=tel_donateur,      # Enregistre le téléphone
                    paroisse_id=p_id,
                    date_messe=date_fr,
                    heure_messe=h_brute,
                    type_messe=type_m,
                    nature_id=n_id,              # Enregistre MAG, MRA ou IS
                    texte=t_txt,
...
def etape_paiement(groupe_id):
    ...
    if not intentions: return redirect(url_for('demande'))
    ...
    if request.method == 'POST':
        ...
        if photo and id_transac:
            # Sécurité : On vérifie si l'ID transaction existe déjà pour un AUTRE groupe
            deja_utilise = Intention.query.filter(Intention.id_transaction == id_transac, Intention.groupe_id != groupe_id).first()
            if deja_utilise:
...
def login_paroisse():
    if request.method == 'POST':
        ...
        if paroisse:
            # On redirige vers l'URL de modification de CETTE paroisse
...
def update_paroisse(id):
    paroisse = db.session.get(Paroisse, id)
    if not paroisse: return redirect(url_for('login_paroisse'))
    ...
    if request.method == 'POST':
        try:
            ...
            if file and file.filename != '':
                ...
                paroisse.logo_url = '/static/uploads/' + filename
...
def gestion_intentions(paroisse_id):
    ...
    # Conversion pour correspondre au format "15/03/2026" vu sur vos photos
    ...
    # TEST : Affiche dans le terminal pour vérifier les noms des colonnes
    if intentions:
        print(f"DEBUG: Donateur={intentions[0].donateur}, Texte={intentions[0].texte}")
...
def admin_prive():
    if request.method == 'POST':
        ...
        if file and file.filename != '':
            ...
            file.save(os.path.join(upload_folder, filename))
            logo_path = '/static/uploads/' + filename

        # Création de l'objet (Maintenant logo_url fonctionnera)
        p = Paroisse(
            nom=nom, 
            ville=ville, 
            code=code, 
            num_mtn=num_mtn, 
            num_moov=num_moov, 
            num_celtiis=num_celtiis,
            logo_url=logo_path  # <--- Cette ligne ne causera plus d'erreur
        )
...
if __name__ == '__main__':
    ...
    # Lancement sur 0.0.0.0 pour l'accès via mobile sur le même réseau Wi-Fi
